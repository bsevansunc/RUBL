...
source('C:/Users/Brian/Desktop/rubl_2_14/scripts/load_env.R')

env.stack = load.env()

# Samples with data:

source('C:/Users/Brian/Desktop/rubl_2_14/scripts/load_swd.R')

swd.list = load.swd()

#-------------------------------------------------------------------------------
# Add tmin^2 and interactions between ppt, pasture, grass, and rowcrop 
#-------------------------------------------------------------------------------

env.stack[['tmin']] = (env.stack[['tmin']]+22.8233)/(cellStats(env.stack[['tmin']],max)+22.833)
env.stack[['tmin2']] = env.stack[['tmin']]^2
env.stack[['ppt']] = (env.stack[['ppt']])/(cellStats(env.stack[['ppt']],max))
env.stack[['ppt.pasture']] = env.stack[['ppt']]*env.stack[['pasture']]
env.stack[['ppt.grass']] = env.stack[['ppt']]*env.stack[['grass']]
env.stack[['ppt.rowcrop']] = env.stack[['ppt']]*env.stack[['rowcrop']]

names(env.stack) # Just to verify


# Add additional environmental variables to swd, as above:

for (i in 1:length(swd.list)){
  swd.list[[i]]$tmin = (swd.list[[i]]$tmin+22.8233)/(max(swd.list[[i]]$tmin)+22.833)
  swd.list[[i]]$tmin2 = swd.list[[i]]$tmin^2
  swd.list[[i]]$ppt = (swd.list[[i]]$ppt)/(max(swd.list[[i]]$ppt))
  swd.list[[i]]$ppt.pasture = swd.list[[i]]$ppt*swd.list[[i]]$pasture
  swd.list[[i]]$ppt.grass = swd.list[[i]]$ppt*swd.list[[i]]$grass
  swd.list[[i]]$ppt.rowcrop = swd.list[[i]]$ppt*swd.list[[i]]$rowcrop
  }
